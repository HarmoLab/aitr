...
#         super(MLP, self).__init__()
#         # TODO ここに中間層，出力層を定義
#         unit_list = [1024, 512, 256, 128, 64]
#         fc_list = []
#         fc_list.append(nn.Linear(28 * 28, unit_list[0]))
#         for i, u in enumerate(unit_list[:-1]):
#             fc_list.append(nn.Linear(unit_list[i], unit_list[i+1]))
#         fc_list.append(nn.Linear(unit_list[-1], 10))
...
#         x = x.view(-1, 28 * 28)
#         # TODO ここに中間層，出力層の処理を書く
#         for i, fc in enumerate(self.fc_list[:-1]):
...
#         fc_seq.add_module("fc1", nn.Linear(28 * 28, unit_list[0]))
#         fc_seq.add_module("relu1", nn.ReLU())
#         for i, u in enumerate(unit_list[:-1]):
#             fc_seq.add_module("fc{}".format(i+2), nn.Linear(unit_list[i], unit_list[i+1]))
#             fc_seq.add_module("relu{}".format(i+2), nn.ReLU())
#         fc_seq.add_module("fc_output", nn.Linear(unit_list[-1], 10))
#         self.fc_seq = fc_seq
#
#     def forward(self, x):
#         '''
